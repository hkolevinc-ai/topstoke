...
def configure_proxy(config: Config) -> None:
    """Load the Worker endpoint and token without ever writing the token to logs."""
    global _PROXY_URL, _PROXY_TOKEN
    _PROXY_URL = os.environ.get("TOPSTOKEE_PROXY_URL", config.proxy_url).strip().rstrip("/")
    _PROXY_TOKEN = os.environ.get("TOPSTOKEE_PROXY_TOKEN", "").strip()
    if _PROXY_URL and not _PROXY_TOKEN:
        raise RuntimeError(
            "Cloudflare Worker is configured, but GitHub secret "
            "TOPSTOKEE_PROXY_TOKEN is missing"
        )
    if _PROXY_URL:
        logging.info("Cloudflare Worker proxy enabled: %s", _PROXY_URL)


def routed_request(url: str) -> tuple[str, dict[str, str]]:
    """Route only TopStokee requests through the fixed-host Worker endpoint."""
    if not _PROXY_URL:
        return url, {}
    target = urlsplit(url)
    ...
    if target.scheme not in {"http", "https"} or target.netloc.lower() != site.netloc.lower():
        return url, {}
    path_and_query = urlunsplit(("", "", target.path or "/", target.query, ""))
    worker_url = f"{_PROXY_URL}/proxy?{urlencode({'path': path_and_query})}"
    return worker_url, {"X-Proxy-Token": _PROXY_TOKEN}
...
def get_text(
    ...
    *,
    ...
    request_url, proxy_headers = routed_request(url)
    headers.update(proxy_headers)
    last_error: Exception | None = None
    for attempt in range(5):
        wait_for_request_slot()
        try:
            response = browser_session().get(request_url, headers=headers, timeout=timeout, allow_redirects=True)
